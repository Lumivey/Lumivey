...
export async function checkGuestDossierDbPreflight(input: {
  ...
  try {
    ...
    if (!['postgres:', 'postgresql:'].includes(url.protocol) ||
        ...
        url.pathname !== '/neondb' || url.username !== 'lumivey_discovery_app_preview' ||
...
ORDER BY expected.table_name`);
    ...
    if (rows.some(r => r.resume_owner !== OWNER || r.correction_owner !== OWNER ||
        r.resume_definer !== true || r.correction_definer !== true ||
        !Array.isArray(r.resume_search_path) || !r.resume_search_path.includes(PATH) ||
        !Array.isArray(r.correction_search_path) || !r.correction_search_path.includes(PATH)))
      return 'BLOCKED_UNSAFE_FUNCTIONS';
    // Preliminary only: never substitute for independent actual A/B and security review.
