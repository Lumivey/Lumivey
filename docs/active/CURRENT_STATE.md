...
- Contactgegevens uit een bron moeten aantoonbaar deterministisch doorlopen naar `sourceBacked.contactDetails` en vervolgens Website Brief; dit is nu de eerstvolgende gate vóór v0.
...
- `lib/lumivey/extract-understanding.ts` moet deterministische contactfeiten uit SourceContext nu ook zelfstandig behouden in `sourceBacked.contactDetails`, zodat een model ze niet stil kan laten verdwijnen.
...
5. Nu: contact/source-datapad hard maken en verifiëren in Website Brief.
6. Daarna: Website Brief → v0 en website beoordelen tegen de goedgekeurde Adrie Preview.
7. Daarna: adaptieve Adrie-gesprekstest zodat antwoorden werkelijk aansluiten op Lumivey's vragen.
8. Daarna correction propagation en Preview-feedback terug naar Discovery.
9. Daarna generieke website-QA (>85% t.o.v. goedgekeurde Preview) invoeren.
...
Doel: verifieer dat contactgegevens uit Firecrawl deterministisch in `sourceBacked.contactDetails` terechtkomen en vervolgens zichtbaar in de Website Brief worden voorgesteld. Zodra dat klopt, gebruik de goedgekeurde Adrie Run-2 Preview als design authority en ga door naar v0. Productie moet de professionele consultantpropositie dominant houden: fotografie/camera is een persoonlijk herkenningsmotief, geen dienst en geen visuele hoofdrol op meerdere plekken.
