...
// Two parallel 15s asset checks + 20s chat read + 90s message submission can exceed 120s.
// Leave room for controlled error handling; never retry an ambiguous message submission.
...
type HandoffStage = "verify-images" | "read-chat" | "send-message" | "done";
...
export async function POST(request: Request) {
  ...
  const requestedVersion = data?.versionId;
  if (data?.confirm !== MARKER || typeof requestedVersion !== "string" || !VERSION_ID.test(requestedVersion)
      || !validBlob(referenceUrl, "visual-reference") || !validBlob(heroUrl, "hero-source") || referenceUrl === heroUrl) {
  ...
  let stage: HandoffStage = "verify-images";
  const logStage = (result: "ok" | "failed" | "unconfirmed", httpStatus?: number) => {
    // Metadata only: no prompts, image URLs, tokens, customer content or upstream error bodies.
    console.info("Lumivey visual handoff stage", JSON.stringify({
      stage, result, elapsedMs: Math.round(performance.now() - phaseStarted),
      totalMs: Math.round(performance.now() - started), ...(httpStatus ? { httpStatus } : {}),
    }));
  };
  try {
    ...
    if (!current.ok) {
      logStage("failed", current.status);
      return NextResponse.json({ error: "Bestaande chat onbereikbaar. Niets verstuurd." }, { status: 424 });
    }
    ...
    // Compare the exact case-sensitive ID obtained from the previous READ against the latest API response.
    // Never transcribe similar-looking 0/O or I/l into a hardcoded source constant.
    if (chat.latestVersion?.id !== requestedVersion || chat.latestVersion?.status !== "completed") {
      logStage("failed", 409);
    ...
    if (!sent.ok) {
      logStage("unconfirmed", sent.status);
      return NextResponse.json({ error: "v0 heeft het bericht niet bevestigd; status eerst uitlezen, niet opnieuw versturen.", upstreamStatus: sent.status }, { status: 502 });
    }
    logStage("ok");
    stage = "done";
    return NextResponse.json({ submitted: true, chatId: CHAT_ID, previousVersionId: requestedVersion, designReferenceVerified: true,
      referenceAttached: true, separateHeroAttached: true, qaPassed: false }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    logStage(stage === "send-message" ? "unconfirmed" : "failed");
    console.error("Adrie visual handoff exception category", JSON.stringify({ stage, errorName: error instanceof Error ? error.name : "unknown" }));
