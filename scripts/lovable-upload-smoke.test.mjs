...
test('PUT sends exact bytes, signed metadata headers, and returns file id', async () => {
  ...
  try {
    ...
    const ticket={url:'https://storage.googleapis.com/signed-object',file_id:'ephemeral/test',headers:{'x-goog-content-length-range':'0,262144000','x-goog-meta-user_id':'test'}};
...
test('rejects invalid destination and ticket',()=>{
  assert.throws(()=>validateTicket({url:'https://attacker.example/image',file_id:'id'}),/Unexpected upload destination/);
  assert.throws(()=>validateTicket({url:'https://storage.googleapis.com/image'}),/file_id/);
});
