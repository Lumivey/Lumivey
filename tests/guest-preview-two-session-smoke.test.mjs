...
const run = extra => spawnSync(process.execPath, [fileURLToPath(script)], {
...
test('even a private passfile cannot override absent independent branch/host confirmation', () => {
  ...
  try {
    ...
    assert.equal(result.status, 2);
    assert.match(result.stderr, process.platform === 'win32'
      ? /BLOCKED_MISSING_SAFE_CREDENTIAL_HANDOFF/ : /BLOCKED_PREVIEW_IDENTITY_UNCONFIRMED/);
    assert.doesNotMatch(result.stdout + result.stderr, /test-only-placeholder|PASS_/);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});

test('operator smoke is read-only and refuses native Windows without verifiable POSIX passfile permissions', () => {
  const source = readFileSync(fileURLToPath(script), 'utf8');
  ...
  assert.match(source, /\(info\.mode & 0o077\) === 0/);
